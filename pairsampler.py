...
class DistanceWeighted(_Sampler):
    ...
    def forward(self, embeddings, labels):
        with torch.no_grad():
            ...
            weight = torch.clamp(weight, 0, float('inf'))  # 将小于0的数值截为0，保留大于等于0的部分
            weight = weight * (neg_mask * (dist < self.nonzero_loss_cutoff)).float()  # 将对应非负样本的负权重置为0
            # 将置0后的weight再进行clip，避免出现 inf 和 NaN
            weight = torch.clamp(weight, min=torch.finfo(torch.float32).eps, max=float('inf'))
